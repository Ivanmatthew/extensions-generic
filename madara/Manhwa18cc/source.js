...
class Parser {
    ...
    parseChapterList($, mangaId, source) {
        //var _a, _b, _c, _d; // Why is this used?
        const chapters = [];
        let sortingIndex = 0;

        var lastKnownChapterNum = -1;
        // For each available chapter..
        for (const obj of $('li.a-h').toArray()) {
            const objAnchor = $('a.chapter-name', obj);
            const objSpan = $('a.chapter-time', obj);
            const objAnchorHrefText = objAnchor.attr('href');

            const id = objAnchorHrefText.substring(objAnchorHrefText.lastIndexOf('/') + 1);
            const chapName = objAnchor.text(); // trimming is not needed
            const chapNum = Number(chapName.replace(/^\D+/g, ''));

            if (isNaN(chapNum) || (lastKnownChapterNum != -1 && chapNum === 0))
            {
                lastKnownChapterNum -= 1;
                chapNum = lastKnownChapterNum;
            }
            else
            {
                lastKnownChapterNum = chapNum;
            }

            let mangaTime = new Date( (new Date()).setHours(0,0,0,0) );
            const timeSelector = objSpan.text();
